...
As an example, we will re-analyze the `scott` dataset with stricter filtering criteria, requiring the presence of at least ten (non-imputed) data points in addition to five consecutive points; fitting with a maximum of three Gaussians, instead of five; and requiring a better fit than the default settings. 
...
For the sake of time, we use a single model (omitting ensembling; `models = 1`) and only three-fold cross-validation folds (`cv_folds = 3`).
...
ppi = predict_interactions(feat, reference, classifier = "SVM", models = 1, cv_folds = 3)
```
